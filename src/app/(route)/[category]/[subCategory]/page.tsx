...
const Page = async ({ params }: { params: Promise<{ category: string; subCategory: string }> }) => {
  ...
  const SubCategoryList: Subcategory | null = await fetchSingleSubCategory(subCategory, category, GET_SUBCATEGORY_BY_URLS);
  if (!SubCategoryList) {
    notFound();
  }
  ...
  return (
    <>
      ...
      <CategoryPage categoryName={name} description={description || ""} categoryUrl={category} ProductList={SubCategoryList} />
    </>
  );
};
