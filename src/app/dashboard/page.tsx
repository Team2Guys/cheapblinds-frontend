...
async function DashboardMain() {
  ...
  const [records, monthlyStats, weeklyStats] = await Promise.all([get_all_records(), MONTHLYSTATS_HANDLER(token), WEEEKLYSTATSHANDLER(token)])

  const categories = monthlyStats?.orders?.map((item: { month: string, Revenue: number, Orders: number }) => item.month);
  const appointmentsData = monthlyStats?.appointments?.map((item: { Appointments: string, Orders: number }) => item.Appointments || 0);
  const ordersData = monthlyStats?.orders?.map((item: { month: string, Revenue: number, Orders: number }) => item.Orders || 0);
