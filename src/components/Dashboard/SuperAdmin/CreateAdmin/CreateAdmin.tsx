    ...
    <div className="max-w-4xl mx-auto p-6">
      ...
      >
        {({ handleSubmit, values, setValues }) => (
          <Form onSubmit={handleSubmit} className="bg-primary p-6 rounded-lg shadow-lg">
            <h2 className="text-2xl text-center font-bold mb-6">{updateFlag ? 'Edit Admin' : 'Create New Admin'}</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-4">
              {checkboxData.map((checkbox) => (
                <label key={checkbox.name} className="flex items-center gap-2">
                  <Field type="checkbox" name={checkbox.name} className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded" />
