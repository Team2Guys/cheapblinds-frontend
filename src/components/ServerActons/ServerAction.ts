...
import { FIND_ONE_MAIN_CATEGORY_SEO, FIND_ONE_PRODUCT_META, FIND_ONE_SUB_CATEGORY_SEO, FIND_PRODUCT_META, GET_ADMIN_DATA } from "graphql/queries";
import { headers } from 'next/headers';
import { fetchSingeEComProduct, fetchSingeProduct, fetchSingeSubCategory, fetchSingleCategory } from 'config/fetch';
...
export const metaObjecthandler = async (category: ISEO_TAGS, search_params?: SEARCH_PARAMS) => {
  ...
  const ImageUrl = category?.posterImageUrl?.imageUrl || category?.posterImage?.imageUrl || '/assets/images/header/logo.png';
 ...
 const { variant, size } = search_params ?? {};
  ...
  return {
    ...
    alternates: {
      canonical:
        (category?.Canonical_Tag && category?.Canonical_Tag !== '') ? `/${category?.Canonical_Tag}/` : fullurl,
    },
  };
}
...
export const SeoTagsHandlerSubCategory = async (subcategoryname: string, maincategory: string): Promise<Metadata | null> => {
  const subcategory = await fetchSingeSubCategory(subcategoryname, maincategory, FIND_ONE_SUB_CATEGORY_SEO);
...
export const SeoTagsHandlerpproduct = async (productname: string, maincategory: string, subCategory: string, eComerece?: boolean , search_params?: SEARCH_PARAMS): Promise<Metadata | null> => {
  let product;
  if (eComerece) {
    product = await fetchSingeEComProduct(productname, maincategory, subCategory, FIND_ONE_PRODUCT_META);
  } else {
