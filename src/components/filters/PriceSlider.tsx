...
export const PriceSlider = ({ selectedPrice, setSelectedPrice }: PriceSliderProps) => {
  const [range, setRange] = useState<[number, number]>(selectedPrice);

  // Sync local state with parent only if values are different
  useEffect(() => {
    const [minSelected, maxSelected] = selectedPrice;
    const [minRange, maxRange] = range;

    if (minSelected !== minRange || maxSelected !== maxRange) {
      setRange(selectedPrice);
    }
  }, [selectedPrice]);
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>, index: number) => {
    ...
    // Update parent state directly to prevent infinite loop
