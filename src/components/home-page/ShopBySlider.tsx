...
export const ShopBySlider = React.memo(({ categoryList }: { categoryList: CategoryItem[] }) => {
  ...
  const handleNext = useCallback(() => {
    setActiveIndex((prev) => {
      ...
      if (newIndex >= totalSlides + centerOffset) return centerOffset;
      return newIndex;
    });
  ...
  const handlePrev = useCallback(() => {
    setActiveIndex((prev) => {
      const newIndex = prev - 1;
      if (newIndex < centerOffset) return totalSlides + centerOffset - 1;
      return newIndex;
    });
  }, [totalSlides, centerOffset]);
  ...
  const getPositionClass = useCallback(
    (index: number) => {
      ...
      if (diff === 0) return "z-40 scale-100 translate-x-0 opacity-100 blur-0 grayscale-0";
      if (Math.abs(diff) === 1) {
        const tx =
          diff > 0
  ...
  const slideElements = useMemo(
    ...
    [extendedCategoryList, getPositionClass, activeIndex],
  );
