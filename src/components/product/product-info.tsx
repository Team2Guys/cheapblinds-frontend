...
const ProductInfo = () => {
  return (
    <div className='space-y-6'>
      <h2>From: D <span className='font-semibold'>299.25</span></h2>
      <p>A luxurious medium-weight chenille in a rose pink colourway, which makes Romans hang well and windows look great.</p>
      <Calculation />
      <HelpingModal />
      <RecessSelector />
      <button className='bg-primary px-4 py-3 rounded-md w-full font-semibold cursor-pointer hover:bg-primary/80'>Get price</button>
      <PaymentMethod installments={200} showheading />
       <div className="bg-primary p-4 rounded-md flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <button className="flex items-center gap-2 bg-white font-semibold px-6 py-2 rounded-md shadow hover:bg-gray-100 cursor-pointer whitespace-nowrap">
          <span className='bg-primary text-white h-6 w-6 flex items-center justify-center rounded-md'>+</span>
          Free Sample
        </button>
      </div>
       <div className="border border-secondary p-4 rounded-md flex  gap-4">
        ...
        <div>
          ...
          <p>Free delivery on orders above 1000 in Dubai – no hidden charges, just doorstep convenience.</p>
        </div>
      </div>
    </div>
  )
